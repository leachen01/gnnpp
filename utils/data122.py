...
class ZarrLoader1:
    ...
    def load_data_1(
        ...
        print(self.stations_rf['station_id'].unique())
...
def load_dataframes_1(
    ...
    # Load Data ######################################################################
    DATA_FOLDER = f"/mnt/sda/Data2/gnnpp-data/122dataframes_{leadtime}"
    res = defaultdict(lambda: None)

    DATA_FOLDER = os.path.join(DATA_FOLDER, "final_train_122")
    # Training data
    TRAIN_RF_PATH = os.path.join(DATA_FOLDER, "train_rf_final_122.pkl")
    TRAIN_RF_TARGET_PATH = os.path.join(DATA_FOLDER, "train_rf_target_final_122.pkl")

    VALID_RF_PATH = os.path.join(DATA_FOLDER, "valid_rf_final_122.pkl")
    VALID_RF_TARGET_PATH = os.path.join(DATA_FOLDER, "valid_rf_target_final_122.pkl")
    # Test on Reforceasts
    TEST_RF_PATH = os.path.join(DATA_FOLDER, "test_rf_final_122.pkl")
    TEST_RF_TARGET_PATH = os.path.join(DATA_FOLDER, "test_rf_target_final_122.pkl")
    # Test on Forecasts
    TEST_F_PATH = os.path.join(DATA_FOLDER, "test_f_final_122.pkl")
    TEST_F_TARGET_PATH = os.path.join(DATA_FOLDER, "test_f_target_final_122.pkl")

    STATIONS_PATH = os.path.join(DATA_FOLDER, "stations_122.pkl")
    ...
    if (os.path.exists(TRAIN_RF_PATH)
        ...
        loader = ZarrLoader1("mnt/sda/Data2/gnnpp-data/EUPPBench-stations")

        print("[INFO] Loading data...")
        df_train, df_train_target, df_f, df_f_target, df_rf, df_rf_target = loader.load_data_1(
...
def load_dataframes_old(
    ...
    if mode == "train" or mode == "eval":
        ...
        TRAIN_RF_TARGET_PATH = os.path.join(DATA_FOLDER, "train_rf_target_final_122.pkl")
        # Test on Reforceasts
        TEST_RF_PATH = os.path.join(DATA_FOLDER, "valid_rf_final_122.pkl")
        TEST_RF_TARGET_PATH = os.path.join(DATA_FOLDER, "valid_rf_target_final_122.pkl")
        # Test on Forecasts
        TEST_F_PATH = os.path.join(DATA_FOLDER, "test_f_final_122.pkl")
        TEST_F_TARGET_PATH = os.path.join(DATA_FOLDER, "test_f_target_final_122.pkl")
    ...
    if mode == "hyperopt":
        # Training data
        TRAIN_RF_PATH = os.path.join(DATA_FOLDER, "train_rf_122.pkl")
        TRAIN_RF_TARGET_PATH = os.path.join(DATA_FOLDER, "train_rf_target_122.pkl")
        # Test on Reforecasts
        VALID_RF_PATH = os.path.join(DATA_FOLDER, "valid_rf_122.pkl")
        VALID_RF_TARGET_PATH = os.path.join(DATA_FOLDER, "valid_rf_target_122.pkl")
...
def load_distances_1(stations: pd.DataFrame) -> np.ndarray:
    ...
    """
    # Load Distances #################################################################
    if os.path.exists("/mnt/sda/Data2/gnnpp-data/distances_EUPP_122.npy"):
        print("[INFO] Loading distances from file...")
        mat = np.load("/mnt/sda/Data2/gnnpp-data/distances_EUPP_122.npy")
    else:
        print("[INFO] Computing distances...")
        mat = compute_dist_matrix(stations)
        np.save("/mnt/sda/Data2/gnnpp-data/distances_EUPP_122.npy", mat)
    return mat
